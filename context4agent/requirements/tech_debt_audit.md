...
- 无公共基类，调用方无法 `except MDAnalysisError` 统一捕获
- `RuntimeError` vs `Exception` 继承不一致
- 部分函数抛出通用 `ValueError`/`FileNotFoundError` 而非领域特定异常

**建议**：创建 `md_analysis.exceptions` 模块，定义 `MDAnalysisError(Exception)` 基类，所有领域异常继承之。
...
| 函数 | 文件 |
|------|------|
| `detect_water_molecule_indices()` | `utils/StructureParser/WaterParser.py` |
| `get_water_oxygen_indices_array()` | `utils/StructureParser/WaterParser.py` |
| `_read_xyz_metal_z_for_steps()` | `potential/CenterPotential.py` |
| CLI 各 `_cmd_*()` 函数 | `cli/_water.py`, `cli/_potential.py` 等 |
...
### P0 — 立即修复（影响正确性或阻碍开发）
1. [x] 为 `CubeParser.py` 5 个公共函数编写单元测试 — ✅ 2026-03-09 `test/unit/utils/test_cube_parser.py` (22 tests)
2. [x] 为 `main.py` 4 个入口函数编写集成测试 — ✅ 2026-03-09 `test/integration/test_main.py` (8 tests)
3. [ ] 提取 `_float()` Fortran 浮点解析到共享位置

### P1 — 短期改善（提升可维护性和用户体验）
4. [ ] CLI 子菜单 `_cmd_*()` 添加统一 try-except 容错
5. [ ] 引入 `logging` 框架，替换 `print()` 调用
6. [ ] 创建 `MDAnalysisError` 异常基类，统一继承关系
7. [ ] 提取 cube 文件发现逻辑到 `CubeParser.discover_cube_files()`
8. [ ] 为 `thickness_sensitivity_analysis()` 补充测试

### P2 — 中期优化（提升性能和代码质量）
9. [ ] 添加帧级并行化选项（`ProcessPoolExecutor`）
10. [ ] 评估水分析单 pass 合并可行性
11. [ ] CLI 模块测试（mock stdin/stdout）
12. [ ] 集中管理界面标签等魔法字符串为常量
13. [ ] 统一 `_circular_mean` 实现

### P3 — 长期架构演进（需求驱动）
14. [ ] 非正交晶胞支持（仅在用户明确需求时启动）
15. [ ] 配置体系重构（统一 schema + 校验）
16. [ ] 分析模块插件化注册机制
