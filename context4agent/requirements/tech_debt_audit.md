...
| 3 | 2.1 Fortran 浮点解析 | 提取 `_float()` Fortran 浮点解析到共享位置 | [ ] |

### P1 — 短期改善（提升可维护性和用户体验）

| # | 对应章节 | 待办项 | 状态 |
|---|---------|--------|------|
| 4 | 3.2 CLI 无容错包装 | CLI 子菜单 `_cmd_*()` 添加统一 try-except 容错 | [ ] |
| 5 | 3.3 零日志框架 | 引入 `logging` 框架，替换 `print()` 调用 | [ ] |
| 6 | 3.1 异常体系不统一 | 创建 `MDAnalysisError` 异常基类，统一继承关系 | [ ] |
| 7 | 2.3 Cube 文件发现模式 | 提取 cube 文件发现逻辑到 `CubeParser.discover_cube_files()` | [ ] |
...
| 12 | 2.6 魔法字符串 | 集中管理界面标签等魔法字符串为常量 | [ ] |
| 13 | 2.2 Circular Mean | 统一 `_circular_mean` 实现 | [ ] |
