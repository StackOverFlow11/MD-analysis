# `md_analysis.water.WaterAnalysis` 内部实现准则（当前实现口径）

> 适用范围：`src/md_analysis/water/WaterAnalysis/`。
...
- 允许：`WaterAnalysis` -> `md_analysis.utils`
- 禁止：`md_analysis.utils` -> `WaterAnalysis`
- 禁止引入与分析无关的上层耦合。

## 5. 变更同步要求

- 以下变更必须同步到 `context4agent`：
  - 新增/删除公开函数
  - 统计口径变化（包括平均权重与重采样规则）
  - 输出列与单位变化
