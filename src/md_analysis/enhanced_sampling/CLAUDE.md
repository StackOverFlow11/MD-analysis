...
程序化入口：`workflows.run_ti_full_analysis(root_dir, output_dir, ...)`（编排 discover→load→analyze→plot+CSV，支持 `point_slice` 切片选择）。
...
不论从 CP2K 还是其他引擎读，TI/SG 的最小数据需求是：
- λ(t) 约束力时序（a.u.，shape `(N_steps,)`）
- dt（时间步，fs）
- ξ₀ TARGET 值（a.u.）
...
`parse_lambda_series(directory) → LambdaSeries` 两个方法返回（dataclass 物理位置在 `engines.models`）。
