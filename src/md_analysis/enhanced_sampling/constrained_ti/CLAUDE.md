...
程序化入口：workflow 层 wrapper `run_ti_full_from_root(root_dir, output_dir, parser, dir_filter, reverse, equilibration, epsilon_tol_ev, auto_equilibration, point_slice)`，编排 discover→load→analyze→plot+CSV，返回 `TIFullAnalysisReport`（文件路径 + JSON 可序列化诊断数值）。

`TIFullAnalysisReport` 的 per-point 诊断（14 个字段：`point_index` / `xi` / `n_analyzed` / `time_start_fs` / `time_end_fs` / `time_total_fs` / `tau_corr` / `n_eff` / `sem_final_au` / `sem_max_au` / `geweke_z` / `drift_D` / `passed` / `failure_reasons`）是 skill / 下游判定"非平衡漂移 / N_eff 太少 / 遍历性假阳"等失败类别的信号源。

**Strict discovery 差异**：`run_ti_full_from_root` 调 `discover_ti_points(..., strict=True)`，任何被 `dir_filter` 选中但 metadata 解析失败的目录直接抛 `FileNotFoundError`；CLI 312 菜单路径保持 `strict=False`（跳过失败目录并 WARN）。
...
| `workflow.py` | 编排器：`analyze_single_point`, `analyze_standalone`, `analyze_ti`, `standalone_diagnostics`, `run_ti_full_from_root` (workflow wrapper), `TIFullAnalysisReport`, `_parse_point_slice`, CSV 导出 |
