...
- **`formats/cp2k/colvar.py`**：保留 `ColvarRestart = ConstraintMetadata` 和 `LagrangeMultLog = LambdaSeries` 两个 alias（Phase 5b rename 兼容层），canonical 名通过 `engines.models` 暴露
