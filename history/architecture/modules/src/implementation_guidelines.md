...
> 适用范围：`src/` 顶层包（当前核心为 `src/__init__.py`）。
>
> 目标：保持一个稳定、轻量、低耦合的顶层入口，不在该层承载业务算法。

## 1. 角色定位与边界

- `src/` 是**顶层命名空间与聚合入口**，不是业务计算层。
- 顶层负责"把子包暴露出来"，不负责"执行分析逻辑"。
- 任何与结构识别、水统计、分布计算相关的实现，必须下沉到 `src/structure/` 及其子层。
- 目录文档治理采用硬约束：`history/architecture/modules/src/` 必须镜像 `src/`，且每个子目录都需维护
  `interface_exposure.md` 与 `implementation_guidelines.md`。

## 2. 导出稳定性等级

- 顶层导出按"稳定接口"管理：
  - 已进入 `src/__init__.py` 且在 `__all__` 中声明的符号，视为对外契约。
...
- 允许方向：`src` -> `src.structure`
- 禁止反向依赖：`src.structure` 不应依赖 `src` 顶层内部状态
- 禁止跨层耦合：
  - `src` 不直接依赖 `test/`、`history/`、`data_example/`
...
1. 更新 `src/__init__.py` 中的导入与 `__all__`
2. 更新接口文档 `history/architecture/modules/src/interface_exposure.md`
3. 运行导入烟雾测试（至少覆盖 `import src` 与 `from src import ...`）
4. 运行回归测试，确认无破坏性影响
